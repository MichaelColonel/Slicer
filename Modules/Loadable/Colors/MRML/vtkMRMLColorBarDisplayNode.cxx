...
void vtkMRMLColorBarDisplayNode::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  vtkMRMLPrintBooleanMacro(LogarithmicScale);
...
void vtkMRMLColorBarDisplayNode::WriteXML(ostream& of, int nIndent)
{
  ...
  vtkMRMLWriteXMLBooleanMacro(LogarithmicScale, LogarithmicScale);
...
void vtkMRMLColorBarDisplayNode::ReadXMLAttributes(const char** atts)
{
  ...
  vtkMRMLReadXMLBooleanMacro(LogarithmicScale, LogarithmicScale);
...
void vtkMRMLColorBarDisplayNode::CopyContent(vtkMRMLNode* anode, bool deepCopy/*=true*/)
{
  ...
  vtkMRMLCopyBooleanMacro(LogarithmicScale);
  vtkMRMLCopyEndMacro();
}
...
void vtkMRMLColorBarDisplayNode::SetOrientation(int id)
{
  switch (id)
    {
    case 0:
      this->SetOrientation(vtkMRMLColorBarDisplayNode::Horizontal);
      break;
    case 1:
    default:
      this->SetOrientation(vtkMRMLColorBarDisplayNode::Vertical);
      break;
    }
}
...
//-----------------------------------------------------------
void vtkMRMLColorBarDisplayNode::GetTitleTextProperty(vtkTextProperty* titleTextProperty)
{
  ...
  titleTextProperty->SetColor(this->GetTitleTextColorRGB());
  titleTextProperty->SetOpacity(this->GetTitleTextOpacity());
  titleTextProperty->SetFontSize(this->GetTitleFontSize());

  std::string fontFamilyName = this->GetTitleFontName();
  if (fontFamilyName == "Arial")
    {
    titleTextProperty->SetFontFamilyToArial();
    }
  else if (fontFamilyName == "Times")
    {
    titleTextProperty->SetFontFamilyToTimes();
    }
  else if (fontFamilyName == "Courier")
    {
    titleTextProperty->SetFontFamilyToCourier();
    }
  else
    {
    titleTextProperty->SetFontFamilyAsString(fontFamilyName.c_str());
    }

  titleTextProperty->SetBold(this->GetTitleFontBold());
  titleTextProperty->SetItalic(this->GetTitleFontItalic());
  titleTextProperty->SetShadow(this->GetTitleFontShadow());

}

//-----------------------------------------------------------
void vtkMRMLColorBarDisplayNode::GetLabelsTextProperty(vtkTextProperty* labelsTextProperty)
{
  if (!labelsTextProperty)
    {
    return;
    }

  labelsTextProperty->SetColor(this->GetLabelsTextColorRGB());
  labelsTextProperty->SetOpacity(this->GetLabelsTextOpacity());
  labelsTextProperty->SetFontSize(this->GetLabelsFontSize());

  std::string fontFamilyName = this->GetLabelsFontName();
  if (fontFamilyName == "Arial")
    {
    labelsTextProperty->SetFontFamilyToArial();
    }
  else if (fontFamilyName == "Times")
    {
    labelsTextProperty->SetFontFamilyToTimes();
    }
  else if (fontFamilyName == "Courier")
    {
    labelsTextProperty->SetFontFamilyToCourier();
    }
  else
    {
    labelsTextProperty->SetFontFamilyAsString(fontFamilyName.c_str());
    }

  labelsTextProperty->SetBold(this->GetLabelsFontBold());
  labelsTextProperty->SetItalic(this->GetLabelsFontItalic());
  labelsTextProperty->SetShadow(this->GetLabelsFontShadow());
}

//-----------------------------------------------------------
void vtkMRMLColorBarDisplayNode::SetTitleTextProperty(vtkTextProperty* titleTextProperty)
{
  if (!titleTextProperty)
    {
    return;
  ...
  this->SetTitleTextColorRGB(titleTextProperty->GetColor());
  this->SetTitleTextOpacity(titleTextProperty->GetOpacity());
  this->SetTitleFontSize(titleTextProperty->GetFontSize());
  this->SetTitleFontName(titleTextProperty->GetFontFamilyAsString());
  this->SetTitleFontBold(titleTextProperty->GetBold());
  this->SetTitleFontItalic(titleTextProperty->GetItalic());
  this->SetTitleFontShadow(titleTextProperty->GetShadow());
...
//-----------------------------------------------------------
void vtkMRMLColorBarDisplayNode::SetLabelsTextProperty(vtkTextProperty* labelsTextProperty)
{
  if (!labelsTextProperty)
    {
    return;
  ...
  this->DisableModifiedEventOn();
  this->SetLabelsTextColorRGB(labelsTextProperty->GetColor());
  this->SetLabelsTextOpacity(labelsTextProperty->GetOpacity());
  this->SetLabelsFontSize(labelsTextProperty->GetFontSize());
  this->SetLabelsFontName(labelsTextProperty->GetFontFamilyAsString());
  this->SetLabelsFontBold(labelsTextProperty->GetBold());
  this->SetLabelsFontItalic(labelsTextProperty->GetItalic());
  this->SetLabelsFontShadow(labelsTextProperty->GetShadow());
